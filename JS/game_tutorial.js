...
$('#putBeeperInfo').popover({
	html: true,
	title: "putBeeper",
	content: "If you want Karel to put down a beeper on his actual position use putBeeper",
	placement: 'putBeeper'
});
...
$('#queriesSurr').popover({
	html: true,
	title: "Queries",
	content: "Queries or also called control statements are used if you want Karel to do something, but only under certain circumstances.<hr> Therto it distinguishes between branches and loops.<br>A branch proves a condition only once and also perform the action that is within this query only once.<br><br>Whereas a loop proves a condition until it is false and so long it will repeat the action within the loop. ",
...
$('#ifInfo').popover({
	html: true,
	title: "if",
	content: "If you want Karel e.g. to move but only if he stands on a beeper, then you can realize this with the 'if-query'.<hr> if( beepersPresent() )<br>&emsp;move();<hr> The query above proves if there's a beeper on Karel's position. If so, Karel will do a single step forward. If not, nothing will happen.",
	placement: 'if'
});
...
$('#elseInfo').popover({
	html: true,
	title: "else",
	content: "The 'else-query' is the contra-part to the 'if-query'. If the 'if-query' isn't true, the 'else-query' will be accomplished; e.g. Karel shall move, if there's no wall in front of him. Otherwise he shall turn to the left:<hr>if( frontIsClear() )<br>&emsp;move();<br>else<br>&emsp;turnLeft();<hr>If there's no wall in front of Karel, he'll only do a single step, but not turn to the left.",
	placement: 'else'
});
...
$('#whileInfo').popover({
	html: true,
	title: "while",
	content: "The 'while-loop' is simmilar to the 'if-query'. It proves a condition and if the condition is true, the function within the loop will be performed. Otherwise not.<br> But the big difference is that the 'while-loop' proves the condition again and again until it is false. Only then it also stops with executing the functions within.",
	placement: 'while'
});
...
$('#bitAndInfo').popover({
	html: true,
	title: "&&",
	content: "This is the AND-Operator.<br>If you want to prove more than one condition, e.g. if Karel is west-facing and if he stands on a beeper, just combine these conditions by the AND-Operator like this:<hr>if( facingWest() && beepersPresent() )<br>&emsp;move();<hr>Both conditions have to be true, otherwise Karel won't move.<br><br><b>Watch out!</b><br>Don't forget to put the conditions in brackets.",
	placement: 'bitAnd'
});
...
$('#bitOrInfo').popover({
	html: true,
	title: "| |",
	content:"This is the OR-Operator.<hr>if( facingWest() || beepersPresent() )<br>&emsp;move();<hr>It also proves more than one condition, but the big difference to the AND-Operator is, that Karel will move if at least one condition is true. Only if none of them is true, he won't move.",
	placement: 'bitOr'
});
...
$('#conditionsSurr').popover({
	html: true,
	title: "Conditions",
	content: "Conditions are used together with queries. They can only be true or false.",
	placement: 'right'
});
...
$('#negConditionsSurr').popover({
	...
	content: "Negativ conditions are used together with queries. They are the exact opposites of the conditions. They can only be true or false.",
	placement: 'left'
});
